# -*- coding: utf-8 -*-
"""
Created on Thu Nov  2 09:29:57 2017

@author: XM
读取第1步保存的tensorflow record文件，并进行训练
程序中提到的书本均指《面向机器智能的tensorflow实践》
"""
...
def read_tfrecord(serialized, batch_size):
    ...
    image = tf.reshape(record_image, [IMAGE_WIDTH, IMAGE_HEIGHT, 1])
    #将label平化为字符串
...
def find_index_label(label_batch):
    return (tf.map_fn(
        lambda l: tf.where(tf.equal(labels_all, l))[0, 0:1][0],
        label_batch,
        dtype=tf.int64))
...
with tf.name_scope("conv1") as scope:
    ...
    conv2d_layer_one = tf.contrib.layers.convolution2d(
        ...
        stride=(2, 2),
        trainable=True)
    print('be caution, 1st later the shape is ', conv2d_layer_one.get_shape())
...
with tf.name_scope("conv2") as scope:
    conv2d_layer_two = tf.contrib.layers.convolution2d(
        ...
        stride=(1, 1),
        trainable=True)
    print('be caution, the shape is ', conv2d_layer_two.get_shape())
...
if __name__ == '__main__':
    ...
    glob_path = glob.glob("./imagenet-dogs/*")
